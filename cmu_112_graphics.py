...
from tkinter import *
from tkinter import messagebox, simpledialog, filedialog, Button, Frame
...
class App(object):
    ...
    # def makeButton(app, x, y, text, command, side):
    #     if side == "Top":
    #         frame = app.topBar
    #     elif side == "Bottom":
    #         frame = app.bottomBar
    ...
    def run(app):
        ...
        # app.topBar = Frame(root).pack(side=TOP, fill=BOTH, expand=YES)
        # create the canvas
        root.canvas = app._canvas = WrappedCanvas(app)
        app._canvas.pack(fill=BOTH, expand=YES)

        #create the bottom and top button nav bar Frames
        # app.bottomBar = Frame(root).pack(side=BOTTOM, fill=BOTH, expand=YES)
